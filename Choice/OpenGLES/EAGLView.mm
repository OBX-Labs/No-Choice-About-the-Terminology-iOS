...
@synthesize animating, animationFrameInterval, displayLink, animationTimer, choice, isPerformance;
...
- (void) setup
{
    ...
    tFont = [[OKTessFont alloc] initWithControlFile:fontName scale:1.0 filter:GL_LINEAR];
    [tFont setColourFilterRed:0.0 green:0.0 blue:0.0 alpha:1.0];
    
    cText = [[OKTextObject alloc] initWithText:textFile withTessFont:tFont andCanvasSize:self.frame.size];
    choice = [[Choice alloc] initWithTessFont:tFont text:cText andBounds:self.frame];
}
    ...
    for(UITouch *touch in touches)
    {
        ...
        if(fID == -1)
        {
            ...
            if(nfID != -1)
            {
                ...
                [choice touchesBegan:(nfID + 1) atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
            }
        }
    }
...
{    
    for(UITouch *touch in touches)
    {
        ...
        {
            CGPoint pt = [touch locationInView:self];
            [choice touchesMoved:(fID + 1) atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
        }
    }
}
...
{   
    for(UITouch *touch in touches)
    {
        ...
        {
            CGPoint pt = [touch locationInView:self];
            [choice touchesEnded:(fID + 1) atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
    ...
    if(isPerformance) {
        ...
        if([touch tapCount] == 2) [self toggleAnimation];
    }
...
{    
    for(UITouch *touch in touches)
    {
        ...
        {
            CGPoint pt = [touch locationInView:self];
            [choice touchesCancelled:(fID + 1) atPosition:CGPointMake(pt.x, (self.frame.size.height - pt.y))];
